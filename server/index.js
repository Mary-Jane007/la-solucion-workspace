...
(async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      if (NODE_ENV === "production") {
        console.log(`Productie: frontend + API op PORT ${PORT}. Open de app via een van: ${CORS_ORIGINS.join(", ")}`);
      } else if (!hasDb()) {
        console.log("Let op: DATABASE_URL ontbreekt. Alleen gebruikers fallback werkt (dev).");
      }
    });
  } catch (err) {
